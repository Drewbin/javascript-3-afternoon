...
//Code Here
let totalPopulation = populations.reduce((total, num) => num + total)
...
let orderTotals = orders.map(total => total.price + (total.price * total.tax))
...
let bobsArray = purchases.filter((person)=>person.owner === 'Bob');
bobsPrices = bobsArray.map(function(price){
  return price.price;
});

function total(){
  bobsTotal = 0;
  for (i=0; i<bobsPrices.length; i++){
    bobsTotal = bobsTotal + bobsPrices[i] 
